...
@section('content')
    <div class="max-w-screen-xl px-4 py-1 sm:px-2 lg:px-2 lg:py-1 mx-auto">
        ...
        <div class="bg-stone-100 rounded-xl shadow-lg mt-7 p-4 sm:p-7">
            ...
            <p class="pl-7 pb-3">You can customize the type of bakery and the quantity you want. Feel free to leave notes for
                specific details you'd like us to know.</p>

            <hr class="mt-0 mb-6" style="border-color:#c4b7a6; border-width: 2px;">

            <div class="flex flex-row mt-4 pl-2 pb-3">
                <label for="product" class="font-semibold text-2xl mr-2">
                    Select Product:
                </label>
                <select id="selectedProduct"
                    class="pl-2 pt-1 pb-1 pr-1 mr-20 border border-2 border-gray-400 h-8 w-40 rounded-xl">
                ...
                <div class="flex pl-2">
                    <label for="quantity" class="font-semibold text-2xl mr-2">
                        Quantity:
                    </label>
                    <div class="items-center">
                        <button
                            class="mr-1 font-bold border border-2 border-gray-400 h-8 w-8 rounded-2xl hover:bg-stone-200"
                            id="decreaseQuantity" @click="decreaseQuantity" :disabled="selectedQuantity <= minQuantity">
                            -
                        </button>
                        <input id="quantity" type="integer"
                            class="mr-1 text-center border border-2 border-gray-400 h-8 w-12 rounded-xl"
                            :value="selectedQuantity" :min="minQuantity" :disabled="!selectedProduct" readonly>
                        <button class="font-bold border border-2 border-gray-400 h-8 w-8 rounded-2xl hover:bg-stone-200"
                            id="increaseQuantity" @click="increaseQuantity">
                            +
                        </button>
                    </div>
                </div>
            ...
            <div class="mt-4 pl-2">
                <label for="pickup_date" class="font-semibold text-2xl mr-2">
                    Pick-up Date:
                </label>
                <input type="date" class="pl-3 pb-1 border border-2 border-gray-400 h-8 w-40 rounded-xl"
                    v-model="selectedPickUpDate" :min="minPickUpDate">
            </div>
            ...
            <div class="flex flex-row mt-4 pl-2">
                <label for "description" class="font-semibold text-2xl mb-2 mr-2 block">
                    Note:
                </label>
...
<script>
    ...
    selectedProduct.addEventListener('change', () => {
        productPrice = parseFloat(selectedProduct.options[selectedProduct.selectedIndex].getAttribute(
            'data-price'));
